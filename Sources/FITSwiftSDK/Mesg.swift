...
open class Mesg: Equatable {
    ...
    func expandComponents(containingField: Field, accumulator: Accumulator) throws {
        ...
        let bitstream = try BitStream(values: containingField.values as! [any Numeric], baseType: containingField.baseType)
        ...
        for component in components {
            ...
            let isSigned = (fields[component.fieldNum]?.baseType ?? Factory.createField(mesgNum: mesgNum, fieldNum: component.fieldNum)?.baseType)?.isSignedInteger ?? false
            var value = try bitstream.readBits(component.bits, signed: isSigned)
    ...
    func getDeveloperField(developerDataIdMesg: DeveloperDataIdMesg, fieldDescriptionMesg: FieldDescriptionMesg) -> DeveloperField? {
        ...
        return devFields[developerDataKey]
    }
