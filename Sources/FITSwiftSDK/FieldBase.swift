...
open class FieldBase: Equatable {
    ...
    func write(outputStream: OutputStream) {
        ...
        for value in values {
            if (baseType.isInvalid(value)) {
                outputStream.append(contentsOf: baseType.invalidBytes)
                continue
            }
            
            writeValue(outputStream: outputStream, value: value!, baseType: baseType)
        }
    }
    
    private func writeValue(outputStream: OutputStream, value: Any, baseType: BaseType) {
        switch baseType {
        case .STRING:
    ...
    func hasEqualValues(rhs: FieldBase) -> Bool {
        ...
        let baseType = lhs.getBaseType()
        for (lhsValue, rhsValue) in zip(lhs.values, rhs.values) {
            let isEqual = switch baseType {
    ...
    var size: UInt8 {
        ...
        case .STRING:
            ...
            return size
        default:
            return numValues * baseType.size
        }
    }
    ...
    func sizeAfterAddingValue(index: Int, value: Any?) -> Int {
        ...
        default:
            ...
            return Int(index + 1) * Int(baseType.size)
        }
    }
