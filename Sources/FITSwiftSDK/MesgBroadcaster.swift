...
public class MesgBroadcaster: MesgListener {
    ...
    private var napEventMesgListeners: [ObjectIdentifier : NapEventMesgListener] = [:]
    ...
    public func addListener(_ listener: NapEventMesgListener) {
        let id = ObjectIdentifier(listener)
        napEventMesgListeners[id] = listener
    }
    
    public func removeListener(_ listener: NapEventMesgListener) {
        let id = ObjectIdentifier(listener)
        napEventMesgListeners.removeValue(forKey: id)
    }
    ...
    public func onMesg(_ mesg: Mesg) throws {
        ...
        case .sleepDisruptionOvernightSeverity:
            ...
            return
        case .napEvent:
            if (self.napEventMesgListeners.count == 0) {
                return
            }

            let napEventMesg = NapEventMesg(mesg: mesg)
            try self.napEventMesgListeners.values.forEach { try $0.onMesg(napEventMesg) }
            return
